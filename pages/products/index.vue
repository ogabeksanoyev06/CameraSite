<template>
   <Head>
      <Title>Products | Fireprotection</Title>
   </Head>
               ...
               <div class="sticky top-10 border-r border-grey-200 pr-6">
                  <Accordion type="single" class="w-full" collapsible>
                     <AccordionItem class="border-none" v-for="(item, i) in categories" :key="i" :value="i + 'a'">
                        <AccordionTrigger class="py-2" v-if="item.product_category_id === null"
                           @click="activeSubCategory(item.id)">
                           {{ item.title[$i18n.locale] }}
                        </AccordionTrigger>
                        <AccordionContent>
                           <ul class="">
                              <li class="flex items-center justify-between cursor-pointer py-2"
                                 v-for="(subCategory, i) in subCategory" @click="navigateToSubCategory(subCategory.id)">
                                 <span class="flex-1">
                                    {{ subCategory.title[$i18n.locale] }}
                                 </span>
                                 <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                    xmlns="http://www.w3.org/2000/svg" v-if="subCategory.id == currentSubCategoryId">
                                    <path
            ...
            <div class="">
               <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                  <div v-for="(item, i) in products" data-aos="fade-up" :key="i">
                     <Card :id="item.id" :photo="item.photo[0]" :title="item.title[$i18n.locale]"
                        :subtitle="item.descriptions[$i18n.locale]" />
                  </div>
               </div>
            </div>
         </div>
