<template>
   <div class="flex flex-col gap-2 bg-white rounded-2xl overflow-hidden group">
      <div class="overflow-hidden flex items-center justify-center w-full max-w-[200px] h-[200px] mx-auto">
         <img :src="`${useRuntimeConfig().public.apiBaseUrl}/site/images/products/${photo}`" class="w-full h-full object-contain group-hover:scale-105 transition-300" />
      </div>
      <div class="p-2">
         <div class="flex flex-col items-start gap-4 p-6 rounded-2xl bg-[#F5F5F7]">
            ...
            <Button variant="link" class="!px-0 !py-0 text-black">
               <DialogRoot :open="isModalOpen">
                  ...
                  <DialogPortal>
                     <DialogOverlay @click="isModalOpen = false" class="bg-blackA9 data-[state=open]:animate-overlayShow fixed inset-0 z-30" />
                     <DialogContent
                     ...
                     >
                        ...
                        <div>
                           <label class="text-lg font-medium mb-1 block" for="buy-name">{{ translations['form.input-order-name'] }} </label>
                           <input
                              v-model="first_name"
                              id="buy-name"
                              type="text"
                           ...
                           <label class="mt-4 text-lg font-medium mb-1 block" for="buy-tel">{{ translations['form.input-phone'] }} </label>
                           <input
                           ...
                           <Button :disabled="first_name.length < 1 || phone_number.length < 17" @click="submitForm" class="block w-full mt-6" type="submit">
                              {{ translations['main.buy'] }}
                           </Button>
                        </div>
               ...
               <span>
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                     <path d="M5.83301 5.83301L9.16634 9.99967L5.83301 14.1663" stroke="#020105" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                     <path d="M10.833 5.83301L14.1663 9.99967L10.833 14.1663" stroke="#020105" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                  </svg>
               </span>
            </Button>
         </div>
